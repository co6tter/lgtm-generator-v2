...
- **仕様**:
  - テキスト位置: 中央
  - フォント: 標準ウェイト（400）、視認性を保ちつつ控えめ
  - テキストカラー: 白 60% 不透明度（影なし）
  - テキストサイズ: 画像高さの4%、レスポンシブ対応

#### F-003: 画像コピー機能
- **説明**: 生成した LGTM 画像を直接クリップボードにコピー
- **入力**: 生成された LGTM 画像
- **出力**: クリップボードにコピーされた画像データ
- **用途**: GitHub、GitLab、Slack などに Ctrl+V (Cmd+V) で直接貼り付け可能
...
- **NFR-009**: ダークモード対応（システム設定に基づく自動切り替え）

### 3.4 セキュリティ
- **NFR-010**: HTTPS 通信必須
- **NFR-011**: XSS 対策実装
- **NFR-012**: API キーの安全な管理 (環境変数)

### 3.5 保守性
- **NFR-013**: TypeScript による型安全性
- **NFR-014**: コンポーネント単位でのモジュール化
- **NFR-015**: Biome によるコード品質維持
...
| 1.3 | 2025-11-10 | Claude | ダークモード対応、LGTMテキスト仕様更新 |
| 1.4 | 2025-11-18 | Claude | F-003をマークダウンコピーから画像コピー機能に変更 |
